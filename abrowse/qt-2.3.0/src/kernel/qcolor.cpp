/****************************************************************************
** $Id: qt/src/kernel/qcolor.cpp   2.3.0   edited 2001-01-26 $
**
...
static void get_system_color( const char* pzName, int* r, int* g, int* b )
{
    ...
    static std::map<os::String,long> cColorMap;
    if ( cColorMap.empty() ) {
	for ( int i = 0 ; asColTable[i].name != NULL ; ++i ) {
	    cColorMap[os::String(asColTable[i].name).Lower()] = (asColTable[i].r << 16) | (asColTable[i].r << 8) | (asColTable[i].b);
	}
    }
    std::map<os::String,long>::iterator i = cColorMap.find( os::String(pzName).Lower() );
    if ( i != cColorMap.end() ) {
	*r = ((*i).second >> 16) & 0xff;
	*g = ((*i).second >> 8) & 0xff;
	*b = (*i).second & 0xff;
